    ...
    blockChainFuncs
      ...
      .then(async (res) => {
        ...
        if (subscryptUsername === undefined) {
          ...
          dispatch({
            type: 'LOAD_USER',
            payload: { type: 'user', wallet: res, address: res.address, username: subscryptUsername },
          })
        }
        return res.address
      })
        ...
        if (result.result == true) {
          ...
          if (subscryptAddress === undefined) {
            (await subscrypt).getAddressByUsername(username).then(async (result) => {
              ...
              dispatch({
                type: 'LOAD_USER',
                payload: { username: username, password: password, type: 'user', address: walletAddress },
              })
            ...
            dispatch({
              type: 'LOAD_USER',
              payload: { username: username, password: password, type: 'user', address: subscryptAddress },
            })
          }
          return username
